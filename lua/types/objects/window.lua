...
---@field mods string
---@field leds "CAPS_LOCK"|"NUM_LOCK"|"CAPS_LOCK|NUM_LOCK"

---@class Window
-- Returns a string holding the top of the current key table activation stack,
-- or `nil` if the stack is empty.
-- See [Key Tables](https://wezfurlong.org/wezterm/config/key-tables.html)
-- for a detailed example
---@field active_key_table fun(self: Window): string|nil
-- A convenience accessor for returning the active pane
-- in the active tab of the GUI window.
-- This is similar to `mux_window:active_pane()` but,
-- because it operates at the GUI layer,
-- it can return `Pane` objects for special overlay panes
...
---@field active_workspace fun(self: Window): string
...
-- but represents the fully-populated state of the configuration,
...
---@field get_appearance fun(self: Window): WindowAppearance
...
---@field get_dimensions fun(self: Window): WindowDimensions
-- Returns the text that is currently selected within the specified pane
-- within the specified window formatted with the escape sequences
-- necessary to reproduce the same colors and styling.
-- This is the same text that `window:get_selection_text_for_pane()` would return,
-- except that it includes escape sequences
---@field get_selection_escapes_for_pane fun(self: Window): string
-- Returns the text that is currently selected within the specified pane
-- within the specified window.
-- This is the same text that would be copied to the clipboard if the `CopyTo`
-- action were to be performed.
--
-- Why isn't this simply a method of the `pane` object?
-- The reason is that the selection is an attribute of the containing window,
-- and a given pane can potentially be mapped into multiple windows
...
---@field mux_window fun(self: Window): MuxWindow
-- Performs a key assignment against the window and pane.
-- There are a number of actions that can be performed against a pane in a window
-- when configured via the keys and mouse configuration options.
--
-- This method allows your Lua script to trigger those actions for itself
---@field perform_action fun(self: Window, key_assignment: Action, pane: Pane)
-- Restores the window from the maximized state.
---@see Window.maximize
---@field restore fun(self: Window)
...
-- this is only useful for options that apply to the GUI window, such as rendering the GUI.
--
-- Each call to `window:set_config_overrides()` will emit the `"window-config-reloaded"`
-- event for the window.
--
-- If you are calling this method from inside the handler for `"window-config-reloaded"`
-- you should take care to only call `window:set_config_overrides()`
-- if the actual override values have changed to avoid a loop
---@field set_config_overrides fun(self: Window, overrides: Config)
-- Resizes the inner portion of the window (excluding any window decorations)
-- to the specified width and height
---@field set_inner_size fun(self: Window, width: number, height: number)
-- This method can be used to change the content that is displayed in the tab bar,
-- to the left of the tabs.
--
-- The content is displayed left-aligned and will take as much space as needed
-- to display the content that you set; it will not be implicitly clipped.
...
---@field set_left_status fun(self: Window, string: string)
...
---@field set_position fun(self: Window, x: number, y: number)
-- This method can be used to change the content that is displayed in the tab bar,
-- to the right of the tabs and new tab button.
-- The content is displayed right-aligned and will be clipped from the left edge
-- to fit in the available space.
-- The parameter is a string that can contain escape sequences that change presentation.
--
-- It is recommended that you use wezterm.format to compose the string
---@field set_right_status fun(self: Window, string: string)
...
-- if so, it specifies how long the notification will remain prominently displayed in milliseconds.
-- To specify a timeout without specifying a url, set the url parameter to `nil`.
--
-- The timeout you specify may not be respected by the system, particularly in X11/Wayland environments,
-- and Windows will always use a fixed, unspecified, duration.
-- The notification will persist on screen until dismissed or clicked, or until its timeout duration elapses
---@field toast_notification fun(self: Window, title: string, message: string, url: string?, timeout_milliseconds: number)
-- Toggles full screen mode for the window
---@field toggle_fullscreen fun(self: Window)
-- Returns the id number for the window.
-- The Id is used to identify the window within the internal multiplexer
-- and can be used when making API calls via wezterm CLI to indicate the subject of manipulation
---@field window_id fun(self: Window): number
